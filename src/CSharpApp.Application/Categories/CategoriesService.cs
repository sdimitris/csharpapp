    ...
    : ICategoriesService
{
    ...
    public Task<Result<IReadOnlyCollection<CategoryDto>>> GetAllAsync(int? offset = null, int? limit = null, CancellationToken cancellationToken = default)
        => apiClient.GetAllAsync(offset, limit, cancellationToken);

    public Task<Result<CategoryDto>> GetByIdAsync(int id, CancellationToken cancellationToken = default)
    {
